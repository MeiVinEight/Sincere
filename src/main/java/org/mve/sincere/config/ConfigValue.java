...
public class ConfigValue
{
	...
	public ConfigValue(AbstractContainerEventHandler parent, String key, Font font)
	{
		...
		this.key = key;
		this.font = font;
		int textWidth = this.width - (2 * this.indent) - this.valueWidth - ConfigValue.PADDING;
		...
		this.reset = new Button.Builder(Component.literal("§c×"), this::onReset)
			.bounds(x + this.width - this.indent + ConfigValue.PADDING, y, this.lineHeight, this.lineHeight)
			.tooltip(Tooltip.create(Component.translatable("sincere.config.reset")))
			.build();
	}
	...
	public void draw(GuiGraphics graphics, int x, int y, int mouseX, int mouseY)
	{
		...
		this.text.setWidth(this.width - (2 * this.indent) - this.valueWidth - ConfigValue.PADDING);
		this.reset.setPosition(x + this.width - this.indent + ConfigValue.PADDING, y);
