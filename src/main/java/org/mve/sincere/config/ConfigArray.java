...
public class ConfigArray extends ScrollPanel
{
	...
	protected int getContentHeight()
	{
		...
		for (ConfigValue value : this.value)
			if (value.visible)
				content += value.height + ConfigValue.PADDING;
		return content - ConfigValue.PADDING;
	}
	...
	protected void drawPanel(GuiGraphics guiGraphics, int entryRight, int relativeY, Tesselator tess, int mouseX, int mouseY)
	{
		...
		for (ConfigValue configValue : this.value)
		{
			...
			relativeY += configValue.height + ConfigValue.PADDING;
		}
	}
	...
	public boolean click(double x, double y)
	{
		...
		for (ConfigValue configValue : this.value)
		{
			...
			cy += configValue.height + ConfigValue.PADDING;
		}
		return false;
	}
}
